...
char	*strcpy();
	...
	hed->event.lon= 0.0;
	hed->event.dep= 0.0;
	hed->event.ot.yr= (short)1970;
	hed->event.ot.mo= (short)1;
	hed->event.ot.day= (short)1;
	...
	hed->record.abstime.yr= (short)1970;
	hed->record.abstime.mo= (short)1;
	hed->record.abstime.day= (short)1;
...
#define	MENKE_MAX(a,b)	(((a) > (b)) ? (a) : (b))
#define	MENKE_MIN(a,b)	(((a) < (b)) ? (a) : (b))
 	...
 	char	*data;
{
	...
	if (head->record.type == DOUBLE) {
		...
		while(n_data_pts--)
		{
			dmax= MENKE_MAX(dmax,*dpt);
			dmin= MENKE_MIN(dmin,*dpt);
			++dpt;
		}
		...
		while(n_data_pts--)
		{
			max= MENKE_MAX(max,*fpt);
			min= MENKE_MIN(min,*fpt);
			++fpt;
		}
