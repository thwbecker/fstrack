...
char *argv[];
{
	...
	double GTG[7][7], GTd[7], E, e, ea, eb, Ea, Eb, Emin, Eamin, Ebmin, R, Rmin, GTGmax;
	double soln[7], tlag, sn[7], temp, damping;
	float *c[7];
	int i, j, n, ii, jj, kk, ierror, maxlag, clag[7];
	int lag1, lag1p, lag2, lag2p, lag3, lag3p, lag1min, lag2min, lag3min, first;
	...
	if( argc != 5 ) {
		...
		fprintf(stderr,"\n");
		fprintf(stderr,"purpose: estimate the 4-pulse operator that relates two\n");
		fprintf(stderr,"    timeseries A(t) and B(t)\n");
		fprintf(stderr,"\n");
		fprintf(stderr,"    Suppose A(t) = s(t)*a(t) and B(t) = s(t)*b(t)\n");
		fprintf(stderr,"    where   a(t) = (sum i=0,3) ai delta(t-ti) and\n");
		fprintf(stderr,"                b(t) = (sum i=0,3) bi delta(t-ti)\n");
		...
		fprintf(stderr,"\n");
		fprintf(stderr,"    When using this routine to to solve for a two-layer anisotropy operator\n");
	...
	if( (f3=fopen(argv[4],"w"))==NULL ) {
		fprintf(stderr,"error: %s: cant open e(t)_ah_file <%s> for read\n", argv[0], argv[3]);
		exit(-1);
		}
	...
	fprintf(stdout,"t0\tt1\tt2\tt3\ta0\ta1\ta2\ta3\tb0\tb1\tb2\tb3\tR\n");
	...
	while( xdr_getrecord2(&head1,&data1,&in1) == 0 ) {
		...
		first=TRUE;
		for( lag3p=3; lag3p<=maxlag; lag3p++ )
		for( lag2p=2; lag2p<lag3p;   lag2p++ )
		for( lag1p=1; lag1p<lag2p;   lag1p++ ) {

			lag1=(lag1p); lag2=(lag2p); lag3=(lag3p); 

			c[0]=data1; c[1]=data1; c[2]=data1;
				c[3]=data2; c[4]=data2; c[5]=data2; c[6]=data2;
			clag[0]=lag1; clag[1]=lag2; clag[2]=lag3;
				clag[3]=0; clag[4]=lag1; clag[5]=lag2; clag[6]=lag3;
			sn[0]=(1.0); sn[1]=(1.0); sn[2]=(1.0);
				sn[3]=(-1.0); sn[4]=(-1.0); sn[5]=(-1.0); sn[6]=(-1.0);

			for( ii=0; ii<7; ii++ ) for (jj=ii; jj<7; jj++ ) {
			...
			for( ii=0; ii<7; ii++) GTG[ii][ii] += damping*GTGmax;

			for( ii=0; ii<7; ii++ ) {
				GTd[ii] = (-sn[ii]) * lagdot( c[ii], clag[ii], data1, 0, n );
				}

			gauss(GTG,GTd,7,7,1.0e-6,&ierror,TRUE);
			...
			if( first ) {
				for(ii=0;ii<7;ii++) soln[ii]=GTd[ii];
				Emin=E; Eamin=Ea; Ebmin=Eb; Rmin=R;
				lag1min=lag1; lag2min=lag2; lag3min=lag3;
				first=FALSE;
				}
			else if( R<Rmin ) {
				for(ii=0;ii<7;ii++) soln[ii]=GTd[ii];
		...
		E = 0.0; Ea = 0.0; Eb = 0.0;
		lag1=lag1min; lag2=lag2min; lag3=lag3min;
		clag[0]=lag1; clag[1]=lag2; clag[2]=lag3;
		clag[3]=0; clag[4]=lag1; clag[5]=lag2; clag[6]=lag3;
		for( ii=0; ii<n; ii++ ) {
			e = data1[ii];
