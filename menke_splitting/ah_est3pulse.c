...
char *argv[];
{
	...
	double GTG[5][5], GTd[5], E, e, ea, eb, Ea, Eb, Emin, Eamin, Ebmin, R, Rmin, GTGmax;
	double soln[5], tlag, sn[5], temp, damping;
	float *c[5];
	int i, j, n, ii, jj, kk, ierror, maxlag, clag[5];
	int lag1, lag1p, lag2, lag2p, lag1min, lag2min, first;
	...
	if( argc != 5 ) {
		...
		fprintf(stderr,"\n");
		fprintf(stderr,"purpose: estimate the 3-pulse operator that relates two\n");
		fprintf(stderr,"    timeseries A(t) and B(t)\n");
		fprintf(stderr,"\n");
		fprintf(stderr,"    Suppose A(t) = s(t)*a(t) and B(t) = s(t)*b(t)\n");
		fprintf(stderr,"    where   a(t) = (sum i=0,2) ai delta(t-ti) and\n");
		fprintf(stderr,"                b(t) = (sum i=0,2) bi delta(t-ti)\n");
	...
	if( (f2=fopen(argv[3],"r"))==NULL ) {
		fprintf(stderr,"error: %s: cant open B(t)_ah_file <%s> for write\n", argv[0], argv[3]);
		exit(-1);
		}
	...
	fprintf(stdout,"t0\tt1\tt2\ta0\ta1\ta2\tb0\tb1\tb2\tR\n");
	...
	while( xdr_getrecord2(&head1,&data1,&in1) == 0 ) {
		...
		first=TRUE;
		for( lag2p=2; lag2p<=maxlag;  lag2p++ )
		for( lag1p=1; lag1p<lag2p;    lag1p++ ) {

			lag1=(lag1p); lag2=(lag2p); 

			c[0]=data1; c[1]=data1;
				c[2]=data2; c[3]=data2; c[4]=data2;
			clag[0]=lag1; clag[1]=lag2;
				clag[2]=0; clag[3]=lag1; clag[4]=lag2;
			sn[0]=(1.0); sn[1]=(1.0);
				sn[2]=(-1.0); sn[3]=(-1.0); sn[4]=(-1.0);

			for( ii=0; ii<5; ii++ ) for (jj=ii; jj<5; jj++ ) {
			...
			for( ii=0; ii<5; ii++) GTG[ii][ii] += damping*GTGmax;

			for( ii=0; ii<5; ii++ ) {
				GTd[ii] = (-sn[ii]) * lagdot( c[ii], clag[ii], data1, 0, n );
				}

			gauss(GTG,GTd,5,5,1.0e-6,&ierror,TRUE);
			...
			if( first ) {
				for(ii=0;ii<5;ii++) soln[ii]=GTd[ii];
				Emin=E; Eamin=Ea; Ebmin=Eb; Rmin=R;
				lag1min=lag1; lag2min=lag2;
				first=FALSE;
				}
			else if( R<Rmin ) {
				for(ii=0;ii<5;ii++) soln[ii]=GTd[ii];
